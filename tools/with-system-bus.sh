#!/bin/sh
# with-system-bus.sh - run a program with a temporary D-Bus system daemon
#
# The canonical location of this program is the telepathy-glib tools/
# directory, please synchronize any changes with that copy.
#
# Copyright (C) 2007-2008 Collabora Ltd. <http://www.collabora.co.uk/>
# Copyright (C) 2013 Ispirata S.r.l. <http://www.ispirata.com/>
#
...
usage ()
{
  ...
  echo "" >&2
  echo "If \$WITH_SYSTEM_BUS_FORK_DBUS_MONITOR is set, fork dbus-monitor" >&2
  echo "with the arguments in \$WITH_SYSTEM_BUS_FORK_DBUS_MONITOR_OPT." >&2
...
while test "z$1" != "z--"; do
  ...
  --sleep=*)
    ...
    ;;
  --system)
    dbus_daemon_args="$dbus_daemon_args --system"
    shift
    ;;
  --config-file=*)
...
e=0
DBUS_SYSTEM_BUS_ADDRESS="`cat $me-$$.address`"
export DBUS_SYSTEM_BUS_ADDRESS

if [ -n "$WITH_SYSTEM_BUS_FORK_DBUS_MONITOR" ] ; then
  echo -n "Forking dbus-monitor $WITH_SYSTEM_BUS_FORK_DBUS_MONITOR_OPT" >&2
  dbus-monitor $WITH_SYSTEM_BUS_FORK_DBUS_MONITOR_OPT \
